...
import { Product, Gallery, GalleryImage, StaffMember, Review, Service, SiteSettings, Page, Menu, AdminUser, Customer } from '../types';
...
interface AdminContextType {
  ...
  sendPasswordResetEmail: (customerId: string) => Promise<{ success: boolean; error?: string }>;
  updateCustomerAddress: (customerId: string, addressId: string, updates: any) => Promise<{ success: boolean; error?: string }>;
  updateCustomerEmailPreferences: (customerId: string, preferences: any) => Promise<{ success: boolean; error?: string }>;
  deactivateCustomer: (customerId: string) => Promise<{ success: boolean; error?: string }>;
  reactivateCustomer: (customerId: string) => Promise<{ success: boolean; error?: string }>;
...
export const AdminProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loginAdmin = async (username: string, password: string) => {
    try {
      const user = MOCK_ADMIN_USERS.find((u) => u.username === username && u.isActive);
  ...
  const updateCustomerAddress = async (customerId: string, addressId: string, updates: any) => {
    try {
      const updated = customers.map((c) => {
        if (c.id === customerId) {
          return {
            ...c,
            addresses: c.addresses.map((a) => (a.id === addressId ? { ...a, ...updates } : a)),
          };
        }
        return c;
      });
  ...
  const updateCustomerEmailPreferences = async (customerId: string, preferences: any) => {
    try {
      const updated = customers.map((c) => {
        if (c.id === customerId) {
  ...
  const deactivateCustomer = async (customerId: string) => {
    try {
      const updated = customers.map((c) => (c.id === customerId ? { ...c, isActive: false } : c));
      setCustomers(updated);
      localStorage.setItem('customers', JSON.stringify(updated));
      return { success: true };
    } catch (error) {
  ...
  const reactivateCustomer = async (customerId: string) => {
    try {
      const updated = customers.map((c) => (c.id === customerId ? { ...c, isActive: true } : c));
      setCustomers(updated);
      localStorage.setItem('customers', JSON.stringify(updated));
      return { success: true };
    } catch (error) {
