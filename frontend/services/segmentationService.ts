 ...
 * If a condition is undefined/null, it does not apply.
 */
function matchesRule(customer: Customer, rule: CustomerSegmentRule): boolean {
  ...
  if (
    ...
    (!customer.orders || customer.orders.length < rule.minOrderCount)
  ) {
    return false;
  }
  ...
  if (rule.maxDaysSinceOrder !== undefined) {
    ...
    // If maxDaysSinceOrder is set, customer must have been inactive longer than the threshold.
