...
router.post(
  ...
  [
    ...
    body('streetAddress').trim().notEmpty().withMessage('Street address required'),
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (isDefault) {
        await pool.query(`UPDATE customer_addresses SET is_default = FALSE WHERE customer_id = ? AND type = ?`, [
          customerId,
          type,
        ]);
      }
...
// DELETE address
router.delete('/:customerId/:addressId', requireCustomer, async (req: Request, res: Response) => {
  try {
    const { customerId, addressId } = req.params;
    const authUser = (req as AuthenticatedRequest).authUser;
