...
const resolveApiKey = (apiKeyOverride?: string): string =>
  apiKeyOverride || process.env.EASYPOST_API_KEY || "";
...
export async function getShippingRates(
  ...
  try {
    ...
    if (!shipmentResponse.ok) {
      throw new Error(
        shipmentPayload?.error?.message ||
          `EasyPost error: ${shipmentResponse.status}`,
      );
    }
    ...
    throw new Error(
      `Failed to get EasyPost rates: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function createLabel(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        payload?.error?.message || `EasyPost error: ${response.status}`,
      );
    }
    ...
    throw new Error(
      `Failed to create EasyPost label: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function trackShipment(
  ...
  try {
    ...
    throw new Error(
      `Failed to track EasyPost shipment: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
