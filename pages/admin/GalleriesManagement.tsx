            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-slate-800 p-6 rounded-lg border border-slate-700 self-start">
                    ...
                    <form onSubmit={handleAddGallery} className="flex gap-2 mb-4">
                        <input type="text" value={newGalleryName} onChange={e => setNewGalleryName(e.target.value)} placeholder="New Gallery Name" className="flex-grow min-w-0 p-2 bg-slate-700 border border-slate-600 rounded-md text-white" />
                        <button type="submit" className="bg-sky-500 text-white p-2 rounded-lg flex items-center justify-center hover:bg-sky-600 shrink-0"><PlusIcon className="w-5 h-5" /></button>
                    </form>
                    ...
                    {selectedGalleryId ? (
                        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
                            <h2 className="text-xl font-semibold text-white mb-4">Images in "{galleries.find(g => g.id === selectedGalleryId)?.name}"</h2>
                             <form onSubmit={handleAddImage} className="grid grid-cols-1 md:grid-cols-[1fr,auto,auto] gap-4 mb-6 items-end">
                                <input type="text" value={newImageName} onChange={(e) => setNewImageName(e.target.value)} placeholder="Image Name" className="w-full p-2 bg-slate-700 border border-slate-600 rounded-md text-white" />
                                <div className="flex items-center gap-2">
                                    {newImageUrl && <img src={newImageUrl} className="w-10 h-10 rounded object-cover" />}
                                ...
                                <button type="submit" className="bg-sky-500 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center hover:bg-sky-600 w-full md:w-auto shrink-0">
