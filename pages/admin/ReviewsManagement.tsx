...
import { EditIcon, TrashIcon, PlusIcon, StarIcon } from '../../components/Icons';
...
const ReviewsManagement: React.FC = () => {
    ...
    const openModal = (review?: Review) => {
        setEditingReview(review || { author: '', text: '', rating: 5 });
        setIsModalOpen(true);
    };
    ...
    const handleSave = async () => {
        ...
        if (editingReview.id) {
            await updateReview(editingReview as Review);
        } else {
            await addReview(editingReview as Omit<Review, 'id'>);
        }
        closeModal();
    };
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        setEditingReview(prev => prev ? { ...prev, [name]: type === 'number' ? parseInt(value, 10) : value } : null);
    };
                    ...
                    <tbody>
                        {reviews.map(review => (
                            <tr key={review.id} className="border-t border-slate-700">
                                ...
                                <td className="p-4">
                                    <button onClick={() => openModal(review)} className="text-gray-400 hover:text-sky-400 mr-4"><EditIcon className="w-5 h-5" /></button>
                                    <button onClick={() => deleteReview(review.id)} className="text-gray-400 hover:text-red-500"><TrashIcon className="w-5 h-5" /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {isModalOpen && editingReview && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
                    <div className="bg-slate-800 rounded-lg shadow-2xl p-8 w-full max-w-lg border border-slate-700">
                        <h2 className="text-2xl font-bold text-white mb-6">{editingReview.id ? 'Edit' : 'Add'} Review</h2>
                        <div className="space-y-4">
                            <input type="text" name="author" value={editingReview.author} onChange={handleChange} placeholder="Author Name" className={inputClasses} />
                            <textarea name="text" value={editingReview.text} onChange={handleChange} placeholder="Review Text" className={inputClasses} rows={4}></textarea>
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-1">Rating</label>
                                <input type="number" name="rating" value={editingReview.rating} onChange={handleChange} min="1" max="5" className={inputClasses} />
                            </div>
                        </div>
