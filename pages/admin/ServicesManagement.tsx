                    ...
                    <tbody>
                        {services.map(service => (
                            <tr key={service.id} className="border-t border-slate-700">
                                ...
                                <td className="p-4">
                                    <button onClick={() => openModal(service)} className="text-gray-400 hover:text-sky-400 mr-4"><EditIcon className="w-5 h-5" /></button>
                                    <button onClick={() => deleteService(service.id)} className="text-gray-400 hover:text-red-500"><TrashIcon className="w-5 h-5" /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {isModalOpen && editingService && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
                    <div className="bg-slate-800 rounded-lg shadow-2xl p-8 w-full max-w-lg border border-slate-700">
                        <h2 className="text-2xl font-bold text-white mb-6">{editingService.id ? 'Edit' : 'Add'} Service</h2>
                        <div className="space-y-4">
                            <input type="text" name="title" value={editingService.title} onChange={handleChange} placeholder="Service Title" className={inputClasses} />
                            <textarea name="description" value={editingService.description} onChange={handleChange} placeholder="Description" className={inputClasses} rows={3}></textarea>
                            <input type="text" name="icon" value={editingService.icon} onChange={handleChange} placeholder="Icon Name (e.g., 'shirt')" className={inputClasses} />
                        </div>
