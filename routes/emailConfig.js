...
router.put("/", async (req, res) => {
    try {
        const { provider, fromEmail, fromName, smtpHost, smtpPort, smtpSecure, smtpUsername, smtpPassword, sendgridApiKey, mailgunDomain, mailgunApiKey, } = req.body;
        ...
        const encryptedMailgunKey = mailgunApiKey
            ...
            : null;
        await pool.query(`INSERT INTO email_config (
    ...
    try {
        ...
        try {
            if (config.provider === "smtp") {
                ...
                if (isRender) {
                    console.log("[Email Test] Running on Render - SMTP connections are blocked");
                    return res.json({
                        success: true,
                        message: "Email configuration appears valid. Note: This deployment platform (Render) blocks outbound SMTP connections. To send real emails, use a transactional email service like SendGrid or Mailgun instead.",
          ...
          `,
                ...
                return res.json({
                    success: true,
                    message: "SendGrid configuration validated. Note: Actual email sending is handled by the emailService.",
                    provider: "sendgrid",
                ...
                return res.json({
                    success: true,
                    message: "Mailgun configuration validated. Note: Actual email sending is handled by the emailService.",
                    provider: "mailgun",
