...
router.get("/", async (_req, res) => {
    try {
        const [rows] = await pool.query(`SELECT c.id, c.first_name as firstName, c.last_name as lastName, c.name, c.email, c.phone, c.is_active as isActive,
...
router.get("/:id", async (req, res) => {
    try {
        const [customerRows] = await pool.query(`SELECT id, first_name as firstName, last_name as lastName, name, email, phone, email_preferences as emailPreferences,
       ...
       FROM customers WHERE id = ?`, [req.params.id]);
        ...
        const [addresses] = await pool.query(`SELECT id, type, first_name, last_name, full_name, street_address, city, state, zip_code, 
              country, phone, is_default FROM customer_addresses WHERE customer_id = ?`, [req.params.id]);
        // Normalize address fields to camelCase
        customer.addresses = addresses.map((addr) => ({
            id: addr.id,
            type: addr.type,
        ...
        const [orders] = await pool.query(`SELECT id, order_number as orderNumber, total, status, created_at as date, 
              subtotal, shipping_cost as shippingCost, tax_amount as taxAmount, 
              tracking_number as trackingNumber, order_data as orderData
       FROM orders WHERE customer_id = ? ORDER BY created_at DESC`, [req.params.id]);
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT id, first_name as firstName, last_name as lastName, name, email, phone, is_active as isActive FROM customers WHERE id = ?`, [req.params.id]);
...
router.patch("/:id/toggle-active", async (req, res) => {
    try {
        const [result] = await pool.query(`UPDATE customers SET is_active = !is_active WHERE id = ?`, [req.params.id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: "Customer not found" });
        }
        const [rows] = await pool.query("SELECT id, is_active as isActive FROM customers WHERE id = ?", [req.params.id]);
        return res.json(rows[0]);
    }
    catch (error) {
