...
router.get('/', async (_req, res) => {
    try {
        const [rows] = await pool.query(`SELECT id, first_name as firstName, last_name as lastName, phone,
...
router.get('/:id', async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT id, first_name as firstName, last_name as lastName, phone,
              ...
              created_at as createdAt, last_login as lastLogin
       FROM admins WHERE id = ?`, [req.params.id]);
        if (rows.length === 0) {
...
router.post('/', [
    ...
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM admins WHERE username = ? OR email = ?', [username, email]);
        if (existing.length > 0) {
            return res.status(400).json({ error: 'Username or email already in use' });
        }
        const id = uuidv4();
        const passwordHash = await bcrypt.hash(password, parseInt(process.env.BCRYPT_ROUNDS || '10'));
        await pool.query(`INSERT INTO admins (id, first_name, last_name, phone, username, email, password_hash, role, permissions, is_active)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)`, [
            id,
            firstName || null,
            lastName || null,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (username !== undefined || email !== undefined) {
            ...
            const [existing] = await pool.query(`SELECT id, username, email FROM admins WHERE (${conditions.join(' OR ')}) AND id != ?`, checkValues);
            if (existing.length > 0) {
                const conflicts = [];
                if (username !== undefined && existing.some((u) => u.username === username)) {
                    conflicts.push('username');
                }
                if (email !== undefined && existing.some((u) => u.email === email)) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const [superAdmins] = await pool.query("SELECT COUNT(*) as count FROM admins WHERE role = 'super_admin'");
...
router.patch('/:id/toggle-active', async (req, res) => {
    try {
        const [result] = await pool.query(`UPDATE admins SET is_active = !is_active WHERE id = ?`, [req.params.id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Admin user not found' });
        }
        const [rows] = await pool.query('SELECT id, is_active as isActive FROM admins WHERE id = ?', [req.params.id]);
        return res.json(rows[0]);
    }
    catch (error) {
