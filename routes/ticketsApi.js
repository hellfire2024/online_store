...
import { requireCustomer, getAuthUser, } from "../middleware/auth.js";
const router = Router();
...
router.get("/", async (req, res) => {
    try {
        ...
        const [tickets] = await pool.query(`SELECT * FROM support_tickets ${whereClause} ORDER BY created_at DESC LIMIT 200`, params);
        // Get replies for all tickets
        const ticketIds = tickets.map((t) => t.id);
        let replies = [];
        if (ticketIds.length > 0) {
            const [replyRows] = await pool.query(`SELECT * FROM ticket_replies WHERE ticket_id IN (?) ORDER BY created_at ASC`, [ticketIds]);
            replies = replyRows;
        }
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const [tickets] = await pool.query("SELECT * FROM support_tickets WHERE id = ? OR ticket_number = ?", [id, id]);
        if (tickets.length === 0) {
        ...
        // Get replies
        const [replies] = await pool.query("SELECT * FROM ticket_replies WHERE ticket_id = ? ORDER BY created_at ASC", [ticket.id]);
        res.json({
            id: ticket.id,
            ticketNumber: ticket.ticket_number,
            customerId: ticket.customer_id,
...
router.post("/", requireCustomer, async (req, res) => {
    try {
        const { ticketNumber, customerId, customerName, customerEmail, subject, message, orderId, priority = "medium", } = req.body;
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open')`, [
        ...
        await pool.query(`INSERT INTO ticket_replies (id, ticket_id, author, message) VALUES (?, ?, 'customer', ?)`, [replyId, ticketId, message]);
        // Fetch the created ticket
        const [tickets] = await pool.query("SELECT * FROM support_tickets WHERE id = ?", [ticketId]);
        const ticket = tickets[0];
        res.status(201).json({
            id: ticket.id,
            ticketNumber: ticket.ticket_number,
...
router.post("/:id/replies", requireCustomer, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (author === "customer" && authUser) {
            const [tickets] = await pool.query("SELECT customer_id FROM support_tickets WHERE id = ?", [id]);
        ...
        const replyId = `reply_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        await pool.query(`INSERT INTO ticket_replies (id, ticket_id, author, message) VALUES (?, ?, ?, ?)`, [replyId, id, author, message]);
        // Update ticket's updated_at
        await pool.query(`UPDATE support_tickets SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [id]);
        // Fetch the reply
        const [replies] = await pool.query("SELECT * FROM ticket_replies WHERE id = ?", [replyId]);
...
router.post("/send-ticket-email", async (req, res) => {
    try {
        const { to, subject, ticketNumber, orderId, priority, message, customerInfo, } = req.body;
        if (!to || !subject || !message) {
            res.status(400).json({ error: "Missing required fields" });
            return;
        }
        // Send email to support
        const emailResult = await sendTicketEmail(to, subject, ticketNumber, orderId, priority, message, customerInfo);
        res.json({
