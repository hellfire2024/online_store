    ...
    try {
        ...
        switch (carrier) {
            ...
            case 'shipstation':
                ...
                label = await shipstationService.createLabel(shipmentData, shipmentData.carrierCode, shipmentData.serviceCode);
                break;
            default:
                res.status(400).json({
...
router.get('/track/:trackingId', async (req, res) => {
    try {
        ...
        let tracking;
        if (carrier === 'easypost') {
            tracking = await easypostService.trackShipment(trackingId);
        }
        else if (carrier === 'shippo') {
            tracking = await shippoService.trackShipment(trackingId, carrierCode);
        }
        else if (carrier === 'shipstation') {
            tracking = await shipstationService.trackShipment(trackingId, carrierCode);
        }
        else {
            res.status(400).json({
