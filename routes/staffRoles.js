...
router.get("/", async (_req, res) => {
    try {
        const [rows] = await pool.query("SELECT * FROM staff_roles ORDER BY created_at DESC");
...
router.post("/", async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO staff_roles (key, label, description, created_at)
       VALUES (?, ?, ?, NOW())
       ON DUPLICATE KEY UPDATE label = VALUES(label), description = VALUES(description)`, [key, label, description || null]);
...
router.put("/:key", async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`UPDATE staff_roles SET label = ?, description = ? WHERE key = ?`, [label, description || null, req.params.key]);
...
//# sourceMappingURL=staffRoles.js.map