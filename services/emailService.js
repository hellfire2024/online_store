...
async function initializeTransporter() {
    try {
        ...
        if (config.provider === "smtp") {
            ...
            transporter = nodemailer.createTransport(mgTransport({
                auth: {
...
export async function sendOrderConfirmationEmail(customerEmail, customerName, orderNumber, orderDetails) {
    try {
...
export async function sendShippingNotificationEmail(customerEmail, customerName, orderNumber, trackingNumber, shipper, shippingUrl) {
    try {
        const transport = transporter || (await initializeTransporter());
        if (!transport || !cachedConfig) {
...
export async function sendTicketEmail(supportEmail, subject, ticketNumber, orderId, priority, message, customerInfo) {
    try {
        const transport = transporter || (await initializeTransporter());
        if (!transport || !cachedConfig) {
